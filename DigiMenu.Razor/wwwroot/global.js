...
function deleteCookie(cookieName) {
    document.cookie = `${cookieName}=;expires=Thu, 01 Jan 2000;path=/`;
}
$(document).ready(function () {
    ...
    if (result) {
        ...
        if (result.IsSuccess === true) {
            Success("", result.MetaData.Message, result.IsReload);
        } else {
            ErrorAlert("", result.MetaData.Message, result.IsReload);
        }
