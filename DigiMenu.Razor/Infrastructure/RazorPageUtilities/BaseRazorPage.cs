...
namespace DigiMenu.Razor.Infrastructure.RazorPageUtilities
{
    ...
    public class BaseRazorPage : PageModel
    {
        ...
        {
            if (result.MetaData.Message.Contains("Exception"))
            {
                var msg = JsonConvert.DeserializeObject<MessageException>(result.MetaData.Message);
                result.MetaData.Message = msg.Exception;
